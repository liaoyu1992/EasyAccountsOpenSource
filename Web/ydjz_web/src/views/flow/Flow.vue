  ...
  <div>
    ...
    <div style="height: 80px; background: #fff; margin-left: 20px">
      <div style="margin-top: 10px; float: left">
        <div v-show="this.totalIn != '0'">
          当月总收入： <span style="color: #42b983">￥{{ this.totalIn }}</span>
        </div>
        <div v-show="this.totalOut != '0'">
          当月总支出： <span style="color: #f54949">￥{{ this.totalOut }}</span>
        </div>
        <div v-show="this.handle === 3">当月结余： ￥{{ this.totalEarn }}</div>
        <div v-show="this.handle === 2">
...
export default {
  ...
  data() {
    return {
      ...
      editShow: false,
      actions: [
        { name: "编辑", color: "#39bdfa" },
        { name: "删除", color: "#f54949" },
      ],
      columns: [
        // 第一列
      ],
  ...
  methods: {
    ...
    doCollectFlow(flow) {
      console.log(
        "/flow/collectFlow/" + flow.id + "/" + (flow.collect ? 0 : 1)
      );
      this.$http({
    ...
    onPickerClick({ selectedValues }) {
      ...
      this.chooseMonth =
        ...
        "-" +
        (pickDate.getMonth() + 1).toString().padStart(2, "0");
      this.getMonthFlow();
    ...
    generateReport() {
      ...
      showConfirmDialog({
        title: "确定生成报表吗？",
        message: "确定生成 " + this.chooseMonth + " 月度报表吗？",
      })
        .then(() => {
          this.makeExcel();
        })
